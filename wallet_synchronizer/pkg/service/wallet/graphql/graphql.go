...
func (s *Service) Schema() graphql.Schema {
	...
	//				var wallet *wallet_db.Wallet
	//
	//				_, wallet, err = wallet_create_service.NewService(s.database, wInput).Create()
	//				if err != nil {
	//					return nil, err
	//				}
	//
	...
	rootMutation := graphql.NewObject(graphql.ObjectConfig{
		...
		Fields: graphql.Fields{
			"createWallet": &graphql.Field{
				Type: wallet_db.WalletGraphQL, // the return type for this field
				Args: graphql.FieldConfigArgument{
					"wallet_id": &graphql.ArgumentConfig{
