...
import (
	...
	"wallet-synchronizer/pkg/service/alchemy/wallet_ethereum_balance"
...
func (s *Service) FindOrCreateWalletTokens() (walletTokens []wallet_token_db.WalletToken, err error) {
	...
	var independentWg sync.WaitGroup
	...
	independentWg.Add(1)
	go func() {
		defer independentWg.Done()
		var tokenDb *token_db.Token
		var logFields = logrus.Fields{"wallet_id": s.walletDb.WalletId, "token_id": "ethereum"}

		tokenDb, err = token_find_or_create_service.NewService(s.db, "ethereum", s.alchemyApiKey).FindOrCreateToken()
		if err != nil {
			logrus.WithFields(logFields).WithError(err).Error("cannot find or create token")
			return
		}

		walletBalance, err := wallet_ethereum_balance.GetWalletEthereumWalletBalance(s.walletDb.WalletId, s.alchemyApiKey)
		if err != nil {
			return
		}
		tokenAmount := util_string.CalculateAmount(walletBalance.Result, 18)
		...
		if _, walletToken, err = wallet_token_get_service.NewService(s.db, s.walletDb.WalletId, tokenDb.TokenId).Get(); err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				walletToken = &wallet_token_db.WalletToken{
					...
					TokenAmountHex: walletBalance.Result,
				}
