...
import (
	...
	"strings"
	"time"
	address_status_db "wallet-syncronizer/pkg/database/wallet_token"
	address_transfers_db "wallet-syncronizer/pkg/database/wallet_transaction"
	"wallet-syncronizer/pkg/model/wallet_transaction"
	"wallet-syncronizer/pkg/util"
)

func GetAddressTokenTransfers(db *gorm.DB, address string, addressStatus address_status_db.WalletToken, browser playwright.Browser) (ats []wallet_transaction.AddressTransaction, err error) {
	...
	_, err = page.Goto("https://www.defined.fi/eth/" + addressStatus.TokenId)
	if err != nil {
		return nil, err
	}
	...
	for _, r := range rows {
		...
		at := wallet_transaction.AddressTransaction{
			TxType:        "",
			Price:         0.0,
			Amount:        0.0,
			Total:         0.0,
			AgeTimestamp:  time.Time{},
			Asset:         addressStatus.TokenId,
...
func UpsertAddressTransfers(db *gorm.DB, addressTransfers []wallet_transaction.AddressTransaction) ([]address_transfers_db.WalletTransaction, error) {
	...
	for i := range addressTransfers {
		...
		//}
		addressTransfersDb = append(addressTransfersDb, address_transfers_db.WalletTransaction{
	...
	for j := range addressTransfersDb {
		//var asd address_transfers_db.WalletTransaction
		var err error
		...
		//err = db.Where("WalletId = ? AND TokenId = ?", wallet, addressStatusesDb[j].TokenId).First(&asd).Error
		//if err != nil {
		//	if errors.Is(gorm.ErrRecordNotFound, err) {
		//		if err = db.Create(&addressStatusesDb[j]).Error; err != nil {
		//			return nil, err
		//		}
		//		// inserire nella la tabella delle transazioni che hanno portato a quel token amount x il token contract wallet e l'addressId (etherscan puoi filtrare per contratto e holder wallet_token) (che non esiste)
		//
		//	} else {
		//		return nil, err
		//	}
		...
		//		// aggiorna la tabelle wallet_token con il nuovo token amont e aggiorna la tabella delle transazioni con solo le transazioni nuove (che non esiste vedi sopra)
		//	}
		//}
	}
	return nil, nil
