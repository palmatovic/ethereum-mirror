...
import (
	"auth/pkg/model/json"
	user_list_service "auth/pkg/service/user/list"
	"errors"
...
func (a *Api) List() (status int, response interface{}) {
	...
	httpStatus, users, err := user_list_service.NewService(a.db, pageSize, pageNumber).List()
	if err != nil {
	...
	return httpStatus, json.Response{Data: fiber.Map{"users": users}}
}
