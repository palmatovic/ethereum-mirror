...
import (
	"auth/pkg/api/group_role_resource_perm/create"
	"auth/pkg/api/group_role_resource_perm/delete"
	"auth/pkg/api/group_role_resource_perm/get"
	list "auth/pkg/api/group_role_resource_perm/list"
	"auth/pkg/api/group_role_resource_perm/update"
	"auth/pkg/url/group_role_resource_perm"
...
func (e *Api) Get(ctx *fiber.Ctx) error {
	status, response := get.NewApi(ctx.Locals("uuid").(string), ctx.OriginalURL(), e.DB, ctx.Params(group_role_resource_perm.Id)).Get()
...
func (e *Api) Delete(ctx *fiber.Ctx) error {
	status, response := delete.NewApi(ctx.Locals("uuid").(string), ctx.OriginalURL(), e.DB, ctx.Params(group_role_resource_perm.Id)).Delete()
	return ctx.Status(status).JSON(response)
}
