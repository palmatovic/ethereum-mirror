...
import (
	...
	group_role_resource_perm_update_service "auth/pkg/service/group_role_resource_perm/update"
	"encoding/json"
...
func (a *Api) Update() (status int, response interface{}) {
	...
	httpStatus, groupRoleResourcePermDb, err := group_role_resource_perm_update_service.NewService(a.db, &groupRoleResourcePerm).Update()
	if err != nil {
		logrus.WithFields(a.fields).WithError(err).Errorf("terminated with failure")
