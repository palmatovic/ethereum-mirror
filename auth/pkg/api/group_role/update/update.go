...
import (
	...
	group_role_update_service "auth/pkg/service/group_role/update"
	"encoding/json"
...
func (a *Api) Update() (status int, response interface{}) {
	...
	if err := json.Unmarshal(a.body, &groupRole); err != nil {
	...
	httpStatus, groupDb, err := group_role_update_service.NewService(a.db, &groupRole).Update()
	if err != nil {
		logrus.WithFields(a.fields).WithError(err).Errorf("terminated with failure")
		return httpStatus, util_json.NewErrorResponse(httpStatus, err.Error())
	}
	logrus.WithFields(a.fields).Info("terminated with success")
	return httpStatus, util_json.Response{Data: fiber.Map{"group_role": groupDb}}
}
