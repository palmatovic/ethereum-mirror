...
import (
	"auth/pkg/api/group_role/create"
	"auth/pkg/api/group_role/delete"
	"auth/pkg/api/group_role/get"
	list "auth/pkg/api/group_role/list"
	"auth/pkg/api/group_role/update"
	"auth/pkg/url/group_role"
...
func (e *Api) Get(ctx *fiber.Ctx) error {
	status, response := get.NewApi(ctx.Locals("uuid").(string), ctx.OriginalURL(), e.DB, ctx.Params(group_role.Id)).Get()
...
func (e *Api) Delete(ctx *fiber.Ctx) error {
	status, response := delete.NewApi(ctx.Locals("uuid").(string), ctx.OriginalURL(), e.DB, ctx.Params(group_role.Id)).Delete()
	return ctx.Status(status).JSON(response)
}
