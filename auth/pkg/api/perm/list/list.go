...
import (
	"auth/pkg/model/json"
	perm_list_service "auth/pkg/service/perm/list"
	"errors"
...
func (a *Api) List() (status int, response interface{}) {
	...
	httpStatus, perms, err := perm_list_service.NewService(a.db, pageSize, pageNumber).List()
	if err != nil {
	...
	return httpStatus, json.Response{Data: fiber.Map{"perms": perms}}
}
