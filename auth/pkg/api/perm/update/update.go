...
import (
	...
	perm_update_service "auth/pkg/service/perm/update"
	"encoding/json"
...
func (a *Api) Update() (status int, response interface{}) {
	...
	if err := json.Unmarshal(a.body, &perm); err != nil {
	...
	httpStatus, permDb, err := perm_update_service.NewService(a.db, &perm).Update()
	if err != nil {
		logrus.WithFields(a.fields).WithError(err).Errorf("terminated with failure")
		return httpStatus, util_json.NewErrorResponse(httpStatus, err.Error())
	}
	logrus.WithFields(a.fields).Info("terminated with success")
	return httpStatus, util_json.Response{Data: fiber.Map{"perm": permDb}}
}
