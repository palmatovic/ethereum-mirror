...
import (
	"auth/pkg/api/resource/create"
	"auth/pkg/api/resource/delete"
	"auth/pkg/api/resource/get"
	list "auth/pkg/api/resource/list"
	"auth/pkg/api/resource/update"
	"auth/pkg/url/resource"
...
func (e *Api) Get(ctx *fiber.Ctx) error {
	status, response := get.NewApi(ctx.Locals("uuid").(string), ctx.OriginalURL(), e.DB, ctx.Params(resource.Id)).Get()
...
func (e *Api) Delete(ctx *fiber.Ctx) error {
	status, response := delete.NewApi(ctx.Locals("uuid").(string), ctx.OriginalURL(), e.DB, ctx.Params(resource.Id)).Delete()
	return ctx.Status(status).JSON(response)
}
