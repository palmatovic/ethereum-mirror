...
import (
	"auth/pkg/model/json"
	resource_list_service "auth/pkg/service/resource/list"
	"errors"
...
func (a *Api) List() (status int, response interface{}) {
	...
	httpStatus, resources, err := resource_list_service.NewService(a.db, pageSize, pageNumber).List()
	if err != nil {
	...
	return httpStatus, json.Response{Data: fiber.Map{"resources": resources}}
}
