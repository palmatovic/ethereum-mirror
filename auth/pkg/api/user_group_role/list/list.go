...
import (
	"auth/pkg/model/json"
	user_group_role_list_service "auth/pkg/service/user_group_role/list"
	"errors"
...
func (a *Api) List() (status int, response interface{}) {
	...
	httpStatus, userGroupRoles, err := user_group_role_list_service.NewService(a.db, pageSize, pageNumber).List()
	if err != nil {
	...
	return httpStatus, json.Response{Data: fiber.Map{"user_group_roles": userGroupRoles}}
}
