...
import (
	"auth/pkg/model/json"
	product_list_service "auth/pkg/service/product/list"
	"errors"
...
func (a *Api) List() (status int, response interface{}) {
	...
	httpStatus, products, err := product_list_service.NewService(a.db, pageSize, pageNumber).List()
	if err != nil {
	...
	return httpStatus, json.Response{Data: fiber.Map{"products": products}}
}
