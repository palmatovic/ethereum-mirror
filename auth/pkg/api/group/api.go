...
import (
	"auth/pkg/api/group/create"
	"auth/pkg/api/group/delete"
	"auth/pkg/api/group/get"
	list "auth/pkg/api/group/list"
	"auth/pkg/api/group/update"
	"auth/pkg/url/group"
...
func (e *Api) Get(ctx *fiber.Ctx) error {
	status, response := get.NewApi(ctx.Locals("uuid").(string), ctx.OriginalURL(), e.DB, ctx.Params(group.Id)).Get()
...
func (e *Api) Delete(ctx *fiber.Ctx) error {
	status, response := delete.NewApi(ctx.Locals("uuid").(string), ctx.OriginalURL(), e.DB, ctx.Params(group.Id)).Delete()
	return ctx.Status(status).JSON(response)
}
