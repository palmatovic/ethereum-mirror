...
import (
	"auth/pkg/model/json"
	group_list_service "auth/pkg/service/group/list"
	"errors"
...
func (a *Api) List() (status int, response interface{}) {
	...
	httpStatus, groups, err := group_list_service.NewService(a.db, pageSize, pageNumber).List()
	if err != nil {
	...
	return httpStatus, json.Response{Data: fiber.Map{"groups": groups}}
}
