...
import (
	"auth/pkg/model/json"
	user_product_list_service "auth/pkg/service/user_product/list"
	"errors"
...
func (a *Api) List() (status int, response interface{}) {
	...
	httpStatus, userProducts, err := user_product_list_service.NewService(a.db, pageSize, pageNumber).List()
	if err != nil {
	...
	return httpStatus, json.Response{Data: fiber.Map{"user_products": userProducts}}
}
