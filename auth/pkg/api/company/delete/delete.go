...
import (
	"auth/pkg/model/json"
	company_delete_service "auth/pkg/service/company/delete"
	"errors"
...
func (a *Api) Delete() (status int, response interface{}) {
	...
	var companyId int
	if companyId, err = strconv.Atoi(a.companyId); err != nil {
	...
	httpStatus, company, err := company_delete_service.NewService(a.db, companyId).Delete()
	if err != nil {
