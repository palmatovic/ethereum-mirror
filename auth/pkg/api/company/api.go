...
import (
	"auth/pkg/api/company/create"
	"auth/pkg/api/company/delete"
	"auth/pkg/api/company/get"
	list "auth/pkg/api/company/list"
	"auth/pkg/api/company/update"
	"auth/pkg/url/company"
...
func (e *Api) Get(ctx *fiber.Ctx) error {
	status, response := get.NewApi(ctx.Locals("uuid").(string), ctx.OriginalURL(), e.DB, ctx.Params(company.Id)).Get()
...
func (e *Api) Delete(ctx *fiber.Ctx) error {
	status, response := delete.NewApi(ctx.Locals("uuid").(string), ctx.OriginalURL(), e.DB, ctx.Params(company.Id)).Delete()
	return ctx.Status(status).JSON(response)
}
