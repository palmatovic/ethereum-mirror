...
import (
	"auth/pkg/api/role/create"
	"auth/pkg/api/role/delete"
	"auth/pkg/api/role/get"
	list "auth/pkg/api/role/list"
	"auth/pkg/api/role/update"
	"auth/pkg/url/role"
...
func (e *Api) Get(ctx *fiber.Ctx) error {
	status, response := get.NewApi(ctx.Locals("uuid").(string), ctx.OriginalURL(), e.DB, ctx.Params(role.Id)).Get()
...
func (e *Api) Delete(ctx *fiber.Ctx) error {
	status, response := delete.NewApi(ctx.Locals("uuid").(string), ctx.OriginalURL(), e.DB, ctx.Params(role.Id)).Delete()
	return ctx.Status(status).JSON(response)
}
