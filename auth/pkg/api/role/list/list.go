...
import (
	"auth/pkg/model/json"
	role_list_service "auth/pkg/service/role/list"
	"errors"
...
func (a *Api) List() (status int, response interface{}) {
	...
	httpStatus, roles, err := role_list_service.NewService(a.db, pageSize, pageNumber).List()
	if err != nil {
	...
	return httpStatus, json.Response{Data: fiber.Map{"roles": roles}}
}
