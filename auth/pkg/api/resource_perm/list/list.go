...
import (
	"auth/pkg/model/json"
	resource_perm_list_service "auth/pkg/service/resource_perm/list"
	"errors"
...
func (a *Api) List() (status int, response interface{}) {
	...
	httpStatus, resourcePerms, err := resource_perm_list_service.NewService(a.db, pageSize, pageNumber).List()
	if err != nil {
	...
	return httpStatus, json.Response{Data: fiber.Map{"resource_perms": resourcePerms}}
}
