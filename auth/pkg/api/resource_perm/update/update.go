...
import (
	...
	resource_perm_update_service "auth/pkg/service/resource_perm/update"
	"encoding/json"
...
func (a *Api) Update() (status int, response interface{}) {
	...
	httpStatus, groupDb, err := resource_perm_update_service.NewService(a.db, &resourcePerm).Update()
	if err != nil {
		logrus.WithFields(a.fields).WithError(err).Errorf("terminated with failure")
		return httpStatus, util_json.NewErrorResponse(httpStatus, err.Error())
	}
	logrus.WithFields(a.fields).Info("terminated with success")
	return httpStatus, util_json.Response{Data: fiber.Map{"resource_perm": groupDb}}
}
